...
async def sync_hydra(db: AsyncSession = Depends(get_db)):
    """
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Hydra synchronization failed: {str(e)}")
...
async def hydra_status():
    ...
    try:
        ...
        return {"hydra_connected": is_healthy, "status": "healthy" if is_healthy else "unhealthy"}

    except Exception as e:
        logger.error(f"Hydra status check failed: {str(e)}")
        return {"hydra_connected": False, "status": "error", "error": str(e)}
