    ...
    try:
        ...
        if login_request.get("skip"):
            ...
            accept_response = await hydra_client.accept_login_request(
                login_challenge=login_challenge, subject=login_request["subject"]
            )
        ...
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Invalid login challenge: {str(e)}")
    ...
    try:
        ...
        if not user:
            ...
            return RedirectResponse(
                url=(f"/login?login_challenge={login_challenge}" f"&error=invalid_credentials&email={email}"),
                status_code=303,
            )
...
async def consent_page(request: Request, consent_challenge: str = Query(...)):
    ...
    try:
        ...
        if consent_request.get("skip"):
            ...
            accept_response = await hydra_client.accept_consent_request(
                consent_challenge=consent_challenge, grant_scope=consent_request["requested_scope"]
            )
            return RedirectResponse(url=accept_response["redirect_to"], status_code=303)
        ...
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Invalid consent challenge: {str(e)}")


@router.post("/consent")
async def consent_submit(request: Request, consent_challenge: str = Form(...), action: str = Form(...)):
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Consent failed: {str(e)}")
...
async def logout_page(request: Request, logout_challenge: str = Query(...)):
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Invalid logout challenge: {str(e)}")


@router.post("/logout")
async def logout_submit(request: Request, logout_challenge: str = Form(...), action: str = Form(...)):
...
async def google_auth(request: Request, login_challenge: str = Query(...)):
...
async def github_auth(request: Request, login_challenge: str = Query(...)):
