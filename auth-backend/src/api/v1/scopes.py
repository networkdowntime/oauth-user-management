...
async def list_scopes(
    ...
    try:
        scopes, total = await scope_service.list_scopes(
            skip=skip, limit=limit, active_only=active_only, applies_to=applies_to, search=search
        )
...
async def create_scope(scope_data: ScopeCreate, scope_service: ScopeService = Depends(get_scope_service)):
    """Create a new scope."""
    try:
...
async def get_scope(scope_id: UUID, scope_service: ScopeService = Depends(get_scope_service)):
    """Get a scope by ID."""
    try:
...
async def delete_scope(scope_id: UUID, scope_service: ScopeService = Depends(get_scope_service)):
    """Delete a scope."""
    try:
...
async def bulk_activate_scopes(request: ScopeBulkActivate, scope_service: ScopeService = Depends(get_scope_service)):
...
async def bulk_deactivate_scopes(
    request: ScopeBulkDeactivate, scope_service: ScopeService = Depends(get_scope_service)
):
...
async def bulk_delete_scopes(request: ScopeBulkDelete, scope_service: ScopeService = Depends(get_scope_service)):
    """Bulk delete scopes."""
    try:
        scope_ids = [UUID(id_str) for id_str in request.scope_ids]
...
async def get_scopes_for_account_type(account_type: str, scope_service: ScopeService = Depends(get_scope_service)):
...
async def get_services_by_scope(scope_id: UUID, scope_service: ScopeService = Depends(get_scope_service)):
...
async def get_available_services_for_scope(scope_id: UUID, scope_service: ScopeService = Depends(get_scope_service)):
...
async def add_service_to_scope(
    scope_id: UUID, service_id: UUID, scope_service: ScopeService = Depends(get_scope_service)
):
    ...
    try:
        ...
        if not success:
            raise HTTPException(status_code=400, detail="Failed to assign service to scope")
...
async def remove_service_from_scope(
    ...
    try:
        ...
        if not success:
            raise HTTPException(status_code=400, detail="Failed to remove service from scope")
