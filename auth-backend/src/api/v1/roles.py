...
async def get_services_by_role(
    role_id: str,
    role_service: RoleService = Depends(get_role_service)
):
...
async def get_available_services_for_role(
    role_id: str,
    role_service: RoleService = Depends(get_role_service)
):
...
async def add_user_to_role(
    ...
    if not success:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Failed to assign user to role"
        )
...
async def add_service_to_role(
    ...
    if not success:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Failed to assign service to role"
        )
...
async def remove_service_from_role(
    ...
    if not success:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Failed to remove service from role"
        )
