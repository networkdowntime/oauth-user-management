...
class Settings(BaseSettings):
    ...
    # JWT settings
    jwt_private_key_path: Path = Field(default=Path("keys/jwt_private.pem"), description="Path to JWT private key file")
    jwt_public_key_path: Path = Field(default=Path("keys/jwt_public.pem"), description="Path to JWT public key file")
    ...
    allowed_hosts: List[str] = Field(default=["*"], description="Allowed host names")
    ...
    # Hydra settings
    hydra_admin_url: str = Field(default="http://hydra:4445", description="Hydra admin API URL")
    hydra_public_url: str = Field(default="http://hydra:4444", description="Hydra public API URL")

    # Default admin user
    default_admin_email: str = Field(default="admin@example.com", description="Default admin user email")
    default_admin_password: str = Field(default="admin_password_change_me", description="Default admin user password")
    ...
    apple_client_secret: Optional[str] = Field(default=None, description="Apple OAuth client secret")

    # Hydra integration settings
    hydra_admin_url: str = Field(default="http://hydra:4445", description="Hydra Admin API URL")
    hydra_public_url: str = Field(default="http://hydra:4444", description="Hydra Public API URL")
    hydra_client_id: str = Field(default="management-ui", description="Hydra client ID for management UI")
    hydra_timeout: int = Field(default=30, description="Hydra API request timeout in seconds")

    # Rate limiting settings
    rate_limit_per_minute: int = Field(default=60, description="Rate limit per minute per IP")
    login_rate_limit_per_minute: int = Field(default=5, description="Login rate limit per minute per IP")
