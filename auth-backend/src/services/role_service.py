...
class RoleService:
    ...
    async def assign_service_to_role(
        ...
        if success:
            ...
            logger.info(f"Role {role.name if role else role_id} assigned to service account {service_account.client_name if service_account else service_id} by {performed_by}")
    ...
    async def remove_service_from_role(
        ...
        if success:
            ...
            logger.info(f"Role {role.name if role else role_id} removed from service account {service_account.client_name if service_account else service_id} by {performed_by}")
