...
from ..schemas.user import UserCreate, UserPasswordReset, UserResponse, UserUpdate
...
class UserService:
    ...
    def __init__(self, user_repo: UserRepository, role_repo: RoleRepository, audit_repo: AuditLogRepository):
    ...
    async def update_user(self, user_id: str, user_data: UserUpdate, performed_by: str) -> Optional[UserResponse]:
    ...
    async def reset_user_password(self, user_id: str, password_data: UserPasswordReset, performed_by: str) -> bool:
        """Reset user password."""
    ...
    async def assign_role_to_user(self, user_id: str, role_id: str, performed_by: str) -> bool:
    ...
    async def remove_role_from_user(self, user_id: str, role_id: str, performed_by: str) -> bool:
    ...
    async def authenticate_user(self, email: str, password: str) -> Optional[User]:
        ...
        try:
            ...
            try:
                ...
                await self.user_repo.update(user, {"last_login_at": datetime.now(), "failed_login_attempts": 0})
                ...
                await self.user_repo.update(user, {"failed_login_attempts": current_attempts + 1})

                logger.warning(
                    f"Password verification failed for user: {email}, failed attempts: {current_attempts + 1}"
                )
                return None
    ...
    async def _log_action(self, action: str, resource_type: str, resource_id: str, details: dict, performed_by: str):
