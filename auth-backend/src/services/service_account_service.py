...
class ServiceAccountService:
    ...
    async def create_service_account(
        self, service_account_data: Dict[str, Any], sync_with_hydra: bool = True
    ) -> ServiceAccount:
        """
        ...
        if client_id:
            ...
            if existing:
                raise ServiceAccountAlreadyExistsError(f"Service account with client_id '{client_id}' already exists")
    ...
    async def update_service_account(
        self, service_account_id: UUID, update_data: Dict[str, Any], sync_with_hydra: bool = True
    ) -> ServiceAccount:
        """
        ...
        # Update in database
        updated_service_account = await self.service_account_repo.update(service_account, update_data)

        try:
            # Sync with Hydra if requested
            if sync_with_hydra:
                hydra_client_data = updated_service_account.to_hydra_client()
                await self.hydra_client.update_client(str(updated_service_account.client_id), hydra_client_data)
                logger.info(
                    f"Successfully synced updated service account {updated_service_account.client_id} with Hydra"
                )
    ...
    async def delete_service_account(self, service_account_id: UUID, sync_with_hydra: bool = True) -> None:
        """
    ...
    async def list_service_accounts(
        ...
        return await self.service_account_repo.search(
            query="", skip=skip, limit=limit  # Empty query to get all, filtered by other params
        )

    async def assign_role_to_service_account(self, service_account_id: UUID, role_id: UUID) -> ServiceAccount:
        """
        ...
        # Assign role
        success = await self.service_account_repo.assign_role(str(service_account_id), str(role_id))
    ...
    async def remove_role_from_service_account(self, service_account_id: UUID, role_id: UUID) -> ServiceAccount:
        """
        ...
        # Remove role
        success = await self.service_account_repo.remove_role(str(service_account_id), str(role_id))
    ...
    async def _update_service_account_scopes(self, service_account: ServiceAccount, scope_ids: List[str]) -> None:
        ...
        if scope_ids:
            # Query scopes by IDs
            result = await self.service_account_repo.db.execute(select(Scope).where(Scope.id.in_(scope_ids)))
    ...
    async def _update_service_account_roles(self, service_account: ServiceAccount, role_ids: List[str]) -> None:
        ...
        if role_ids:
            # Query roles by IDs
            result = await self.service_account_repo.db.execute(select(Role).where(Role.id.in_(role_ids)))
