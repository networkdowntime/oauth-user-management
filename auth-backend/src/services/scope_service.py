...
from ..schemas.scope import ScopeCreate, ScopeUpdate, ScopeResponse, ScopeAppliesTo as SchemaScopeAppliesTo
...
class ScopeService:
    ...
    async def list_scopes(
        ...
        if search:
            search_filter = or_(
                Scope.name.ilike(f"%{search}%"),
                Scope.description.ilike(f"%{search}%")
            )
    ...
    async def bulk_delete_scopes(self, scope_ids: List[UUID]) -> int:
        ...
        result = await self.db.execute(
            delete(Scope).where(Scope.id.in_(scope_ids))
        )
        await self.db.commit()
        return result.rowcount
        ...
        result = await self.db.execute(
            ...
            .where(
                and_(
                    ...
                    Scope.applies_to.like(f"%{applies_to.value}%")
                )
            )
        )
    ...
    async def get_service_accounts_without_scope(self, scope_id: UUID) -> List[Dict[str, Any]]:
        ...
        result = await self.db.execute(
            select(ServiceAccount)
            .where(ServiceAccount.id.not_in(subquery))
        )
        service_accounts = result.scalars().all()
    ...
    async def assign_service_to_scope(self, service_id: UUID, scope_id: UUID, performed_by: str) -> bool:
        ...
        service_result = await self.db.execute(
            select(ServiceAccount).where(ServiceAccount.id == service_id)
        )
