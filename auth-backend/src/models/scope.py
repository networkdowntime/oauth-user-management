...
class Scope(Base):
    ...
    )  # Comma-separated values: "Service-to-service", "Browser", or "Service-to-service,Browser"
    ...
    @classmethod
    def from_hydra_scope(
        cls,
        hydra_scope_name: str,
        description: str = "",
        applies_to: Optional[List[ScopeAppliesTo]] = None,
    ):
        """Create a Scope instance from Hydra scope information."""
        if applies_to is None:
            applies_to = [
                ScopeAppliesTo.SERVICE_TO_SERVICE,
                ScopeAppliesTo.BROWSER,
            ]  # Default to both
