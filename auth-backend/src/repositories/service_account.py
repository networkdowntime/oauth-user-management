...
class ServiceAccountRepository(BaseRepository[ServiceAccount]):
    ...
    async def get_by_client_id(self, client_id: str) -> Optional[ServiceAccount]:
        ...
        result = await self.db.execute(
            ...
            .options(selectinload(ServiceAccount.roles), selectinload(ServiceAccount.scopes))
    ...
    async def get_by_id_with_roles(self, id: str) -> Optional[ServiceAccount]:
        """Get service account by ID with roles loaded (alias for get_by_id)."""
        return await self.get_by_id(id)

    async def get_all_with_roles(self, skip: int = 0, limit: int = 100) -> List[ServiceAccount]:
        """Get all service accounts with roles loaded (alias for get_all)."""
    ...
    async def assign_role(self, service_account_id: str, role_id: str) -> bool:
        ...
        service_account = await self.get_by_id_with_roles(service_account_id)
    ...
    async def get_service_accounts_without_role(self, role_id: str) -> List[ServiceAccount]:
        ...
        accounts_without_role = [
            account for account in all_accounts if not any(role.id == role_id for role in account.roles)
        ]
